...
void Squad::addUnitsToMicroManagers()
{
	...
	for (auto & unit : _units)
	{
		if(unit->isCompleted() && unit->getHitPoints() > 0 && unit->exists())
		{
			...
			else if ((unit->getType().groundWeapon().maxRange() > 32) || (unit->getType() == BWAPI::UnitTypes::Protoss_Reaver) || (unit->getType() == BWAPI::UnitTypes::Protoss_Carrier) || (unit->getType() == BWAPI::UnitTypes::Zerg_Scourge))
			{
